...
1. 至 <https://www.docker.com/products/docker-desktop/> 下載 Windows 版並安裝，
   全部使用預設選項（可能要求重新開機）
...
1. 建立資料夾 `goldenai-lite`（位置自選），放入開發方提供的四個檔案：
...
2. 將新 tar 檔放入系統資料夾，開啟指令視窗（方法見「基本操作」）執行：

   ```powershell
   docker load -i goldenai-lite_v1.1.0.tar
   ```

3. 執行 `notepad .env`，將 `GOLDENAI_LITE_TAG` 改為新版本號，存檔
4. 執行：
...
退回舊版：將 `GOLDENAI_LITE_TAG` 改回舊版本號，再執行 `docker compose up -d`。
...
- `logs` 子資料夾中對應的日誌檔：清單問題附 `recommendations_fetcher.log`、
...
（<https://www.sinotrade.com.tw>）登入後操作，為一次性設定。
操作上有疑問時，可洽永豐營業員或客服。
