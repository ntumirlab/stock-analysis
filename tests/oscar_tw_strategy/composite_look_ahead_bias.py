...
OUTPUT_ROOT = REPO_ROOT / "assets" / "OscarTWStrategy" / "composite_look_ahead_bias"


class OscarCompositeLookAheadBiasRunner:
	...
	def _to_json_safe(value):
		if isinstance(value, dict):
			return {str(k): OscarCompositeLookAheadBiasRunner._to_json_safe(v) for k, v in value.items()}
		if isinstance(value, (list, tuple)):
			return [OscarCompositeLookAheadBiasRunner._to_json_safe(v) for v in value]
	...
	def run_once(self) -> Path:
		...
		strategy = OscarCompositeStrategy(config_path=str(self.config_path))
		...
		buy_score = strategy.signal_power.get("composite")
		if buy_score is None:
			raise AssertionError("Composite buy score is missing after strategy run.")
		buy_score.to_csv(out_dir / "buy_score_composite.csv", encoding="utf-8-sig")

		base_position = strategy.base_position
		base_position = base_position.loc[self.start_date:]
...
class TestOscarCompositeLookAheadBias(unittest.TestCase):
	def test_single_dry_run_dump_artifacts(self) -> None:
		runner = OscarCompositeLookAheadBiasRunner()
		runner.run_once()
