...
class MaxStockTestExecutor:
    ...
    def _test_single_config(base_position, n_stocks, fee_ratio=0.001425, tax_ratio=0.003):
        ...
        try:
            ...
            final_position = pd.DataFrame(
                final_position_values, index=base_position.index.copy(), columns=base_position.columns.copy()
            )
    ...
    def _save_results(self, results):
        ...
        summary_df['sharpe_ratio'] = summary_df['sharpe_ratio'].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
        ...
        logger.info("=" * 60)
        logger.info(
            f"最高年化報酬率: max_stocks={df.loc[best_return_idx, 'max_stocks']}, "
            f"報酬率={df.loc[best_return_idx, 'annual_return']:.2%}"
        )

        if best_sharpe_idx is not None:
            logger.info(
                f"最高夏普比率: max_stocks={df.loc[best_sharpe_idx, 'max_stocks']}, "
