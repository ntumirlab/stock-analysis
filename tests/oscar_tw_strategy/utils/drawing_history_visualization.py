...
def create_trading_visualization(
    ...
    position_shifted = (
        position.shift(1).reindex(valid_dates, fill_value=False).fillna(False)
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=sar_values,
            mode="markers",
            marker=dict(size=3, color="#ff9800", symbol="circle"),
            name="SAR",
    ...
    fig.update_yaxes(
        title_text="MACD", row=2, col=1, showgrid=True, gridwidth=1, gridcolor="#e0e0e0"
    )
    ...
    fig.write_html(str(output_path), config=config, include_plotlyjs="cdn")

    return str(output_path)


def prepare_price_data(
    stock_id: str, market_data: dict, start_date: Optional[str] = None
) -> pd.DataFrame:
    """
    準備價格數據供視覺化使用
