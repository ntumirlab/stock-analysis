    ...
    for stock in available_stocks:
        ...
        if len(new_high_dates) > 0:
            plt.scatter(
                ...
                s=50,
                zorder=5,
                label=f'New Highs ({len(new_high_dates)} points)',
            )

        # 設定圖表格式
        plt.title(
            f'{stock} - Price Trend Analysis\nPeriod: {date_range[0].strftime("%Y-%m-%d")} to {date_range[-1].strftime("%Y-%m-%d")}',
            fontsize=14,
            fontweight='bold',
        )
...
def diagnose_strategy(
    ...
    if start_date in buy_signal_dates:
        ...
        if len(valid_dates) == 0:
            ...
            print(
                f"   數據範圍: {buy_signal_dates[0].strftime('%Y-%m-%d')} 到 {buy_signal_dates[-1].strftime('%Y-%m-%d')}"
            )
            return
        ...
        if closest_date != start_date:
            print(
                f"⚠️  指定日期 {start_date.strftime('%Y-%m-%d')} 不在交易日中，使用最接近的交易日 {closest_date.strftime('%Y-%m-%d')}"
            )

    print(
        f"📅 分析日期: {latest_dates[0].strftime('%Y-%m-%d')} 到 {latest_dates[-1].strftime('%Y-%m-%d')} (共{len(latest_dates)}天)"
    )
